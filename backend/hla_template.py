"""
hla_template.py  â  HLA Typing Report PDF Generator
...
Supports three report types that are auto-selected by hla_data_parser:
    single_hla       â NGS single patient layout (1 page)
    transplant_donor â NGS patient + donor(s) layout (1â2 pages)
    rpl_couple       â RPL couple layout (3 pages, fundamentally different)
...
# âââ Page geometry (Letter, 21.59 cm Ã- 27.94 cm) ââââââââââââââââââââââââââââ
PAGE_W, PAGE_H = letter        # 612.0 Ã- 792.0 pts
MARGIN_L = 15 * mm            # â 42.5 pts  â actual content starts ~43 pts
MARGIN_R = 15 * mm
MARGIN_T = 2  * mm
MARGIN_B = 3  * mm
CONTENT_W = PAGE_W - MARGIN_L - MARGIN_R   # â 510 pts
QR_ZONE   = 30 * mm   # blank strip reserved above footer on every page for external QR overlay

# âââ Colour palette â extracted precisely via PyMuPDF from all Manual-Report PDFs â
# NGS
C_NGS_TITLE     = colors.HexColor("#002060")   # NGS title (GillSansMT-Bold 18pt) â exact
C_INFO_BG       = colors.HexColor("#E2E2E2")   # Patient info ALL cells â grey
C_HLA_HDR       = colors.HexColor("#FABF8F")   # HLA table header row â exact
C_HLA_ROW       = colors.HexColor("#F2F2F2")   # HLA data rows â exact
C_APPROVAL      = colors.HexColor("#2C6BAA")   # "Reviewed and approved by" line â exact
# RPL â plain white table, black borders, no fills (confirmed by fitz audit)
C_RPL_TITLE     = colors.HexColor("#001F5F")   # RPL title (SegoeUI-Bold 14pt) â exact
...
# âââ Static text constants ââââââââââââââââââââââââââââââââââââââââââââââââââââ
COVERAGE_LINES = [
...
# Extra coverage lines shown only on the "11 Loci" template â covers the
